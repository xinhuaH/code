...
#endif
#ifdef AKM_CHARGE_CAL
	fd = open("/sys/class/power_supply/battery/current_now", O_RDONLY);
		if (fd < 0) {
		ALOGE("current read error %s", strerror(errno));
			return -1;
	}
	read(fd, value, 32);
	close(fd);
	iCharger = strtol(value,NULL,10);
	iCharger = iCharger/ 1000;
	//ALOGE("iCharger = %d, regx = %d, regy = %d, regz = %d\n",iCharger,regx,regy,regz);

	temp1 = regx;
	temp2 = regy;
	temp3 = regz;
	tempraw = temp1 + (31* iCharger) / 10000;
	bData[1] = tempraw & 0xFF;
	bData[2] = (tempraw >> 8) &0xFF;
	tempraw = temp2 - (40 * iCharger) / 10000;
	bData[3] = tempraw & 0xFF;
	bData[4] = (tempraw >> 8) &0xFF;
	tempraw = temp3 -(50* iCharger) / 10000;
	bData[5] = tempraw & 0xFF;
	bData[6] = (tempraw >> 8) &0xFF;
#endif
