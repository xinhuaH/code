...
/*-----------------------------------------------------------*/
	...
	TCB_t *pxTCB;
        ...
        pxTaskStatus->pxStackTop  = pxTCB->pxTopOfStack;/*Support For CmBacktrace*/
...
/*-----------------------------------------------------------*/
    ...
    /*< Support For CmBacktrace start>*/

    void vTaskStatus(TaskStack_t *ptStackArry, UBaseType_t *ptSize)
    {
        TaskStatus_t *pxTaskStatusArray;
	    volatile UBaseType_t uxArraySize, x;
		...
		if( pxTaskStatusArray != NULL )
		{
			...
			for( x = 0; x < uxArraySize; x++ )
			{
                prvWriteNameToBuffer( &(ptStackArry[x].cTaskName[0]), pxTaskStatusArray[ x ].pcTaskName );
            
                ptStackArry[x].tStackTop = (StackType_t)pxTaskStatusArray[x].pxStackTop;
			}
...
uint32_t  vTaskStackPointer(void)
{
    return (uint32_t)pxCurrentTCB->pxTopOfStack;
}
